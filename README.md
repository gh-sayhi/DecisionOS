...
> Generate 后不会直接输出报告，而是先按 Pack 追问 3-5 个关键问题，补齐决策上下文。

<img src="./docs/images/decisionos-followups.png" alt="DecisionOS 追问机制截图" width="100%" />

### 决策报告

> 报告页展示 Pack 评分、AI 推理时间线、参考案例、风险矩阵、预算、时间线、本周行动清单和导出入口。

<img src="./docs/images/decisionos-report.png" alt="DecisionOS 决策报告截图" width="100%" />
...
```text
Executive Summary
是否上线企业 AI 决策工作台模块应被当作一套决策系统来处理，而不是一次性任务。当前建议是：建议推进并设置验证门。

Decision Verdict
以“发布聚焦版 MVP”作为主路径，并依据 RICE 评分设置明确证据门。
...
Risk Matrix
- 证据质量不足：先定义最小可验证证据包。
- 执行被稀释：第一轮只保留一个负责人、一个核心指标和一个复盘节奏。

This Week Action List
- 周一：完成一页决策简报。
- 周二：访谈 3 个关键用户或相关方。
- 周三：对主路径和备选路径做成本、风险、可逆性评分。
```
